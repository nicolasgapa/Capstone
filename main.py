...
results = []
# files_list = list(range(104461, 109310))
files_list = ['107309']
# random.shuffle(files_list)
for index, f in enumerate(files_list, 1):

    # Inputs.
    file = "data\\training\\{}.csv".format(f)
    ...
    ax2.set_ylabel('Neural network confidence')
    ...
    for e, t in enumerate(range(start_time, ceil(end_time), 1), 0):
        ...
        predictions = []
        for network, s in zip(loaded_networks, [spectra, spectra_2, spectra_3]):
        ...
        if t % signal_processing_interval == 0:
            # Extract the data from the last interval.
    ...
    # Append to results.
    # plt.close()
    print([index, f, label, detections])
    results.append([f, label, detections])
